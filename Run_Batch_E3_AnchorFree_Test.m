% Run_Batch_E3_AnchorFree_Test.m
% E3 production batch: case 2 (E3 anchor-free, anchors_mobile=true).
% 16 nodes, all mobile (no fixed anchors).
%
% Two-phase sweep:
%   Phase 1: fix T_round = T_round_ref, sweep velocity v
%   Phase 2: fix v = v_ref, sweep T_round
%   Cross-point (v_ref, T_round_ref) computed once and shared.
%
% 6-way comparison: SCI+MPLS / EKF+MPLS / EKF+inflate+MPLS / CI+MPLS /
%   SDS-TWR+SCI / DR (Dead Reckoning added vs E2 as lower-bound baseline).
% Success criterion: all cooperative methods beat DR in RMSE.
...
%% 1. Experiment setup
%% ========================================================================
% Full-sweep mode (production, N_MC=100)
velocities     = [5, 10, 15, 20, 25, 30, 35, 40];
T_round_values = [0.016, 0.032, 0.064, 0.096, 0.128, 0.160];   % 6 pts, 10x span
N_sim          = 100;
% Single-point mode (Wilcoxon check, N=50) -- uncomment to use:
% velocities     = [25];
% T_round_values = [0.064];
% N_sim          = 50;
scenario_id    = 2;    % case 2: 16-node heterogeneous (E2/E3)
node_count     = 16;
T_total_val    = 20.0;

% Two-phase reference values (same as E2 for cross-experiment comparison)
v_ref       = 25;      % Phase 2 fixed speed (m/s)
T_round_ref = 0.064;   % Phase 1 fixed polling period (s)
...
settings.verbose_sci            = false;  % suppress in batch (avoids interleaved parfor output)
...
% 6-way method list (E2 5-way + DR)
methods = {'sci_mpls', 'ekf_mpls', 'ekf_inflate_mpls', 'sds_twr_sci', 'ci_mpls', 'dr'};
nM = length(methods);
...
csv_dir   = fullfile('csv',   'E3_AnchorFreeTest', time_tag);
json_dir  = fullfile('json',  'E3_AnchorFreeTest', time_tag);
...
temp_data_dir = fullfile('matfile', 'temp_E3_data');
...
fprintf('============================================================\n');
fprintf('E3: ANCHOR-FREE COOPERATIVE LOCALIZATION (Case %d)\n', scenario_id);
fprintf('Nodes: %d (ALL mobile, no anchors)\n', node_count);
...
fprintf('Methods: 6-way (SCI+MPLS, EKF+MPLS, EKF+infl.+MPLS, SDS-TWR+SCI, CI+MPLS, DR)\n');
fprintf('============================================================\n');
...
for mi = 1:nM
    ...
    % Per-condition raw MC vectors for median/IQR/bootstrap (ANEES is right-skewed)
...
for iv = 1:nV
    ...
    [metrics, dbg, traj] = run_e3_condition(velocities(iv), T_round_ref, ...
...
for jf = 1:nF
    ...
    [metrics, ~, ~] = run_e3_condition(v_ref, T_round_values(jf), ...
        N_sim, scenario_id, node_count, T_total_val, settings, ...
        temp_data_dir, false);
...
result_mat = fullfile('matfile', ...
    sprintf('results_E3_case%d_%s.mat', scenario_id, time_tag));
...
csv_path = fullfile(csv_dir, sprintf('stats_E3_case%d.csv', scenario_id));
...
results_json.meta = struct( ...
    'experiment', 'E3_AnchorFree_Cooperative', ...
...
json_path = fullfile(json_dir, ...
    sprintf('results_E3_case%d.json', scenario_id));
...
%% ========================================================================
fprintf('\n============================================================\n');
fprintf('E3 SUMMARY (MC=%d per condition, Anchor-Free)\n', N_sim);
fprintf('============================================================\n');

fprintf('\n--- Phase 1: T_round = %.3fs ---\n', T_round_ref);
fprintf('%-6s | %-8s %-8s %-8s %-8s %-8s %-8s | %-8s %-8s\n', ...
    'v(m/s)', 'SCI', 'EKF', 'EKFinf', 'SDS', 'CI', 'DR', 'ANEES_S', 'ANEES_E');
fprintf('----------------------------------------------------------------------\n');
for iv = 1:nV
    fprintf('%-6d | %-8.3f %-8.3f %-8.3f %-8.3f %-8.3f %-8.3f | %-8.2f %-8.2f\n', ...
        velocities(iv), ...
        p1_rmse.sci_mpls(iv), p1_rmse.ekf_mpls(iv), ...
        p1_rmse.ekf_inflate_mpls(iv), p1_rmse.sds_twr_sci(iv), ...
        p1_rmse.ci_mpls(iv), p1_rmse.dr(iv), ...
        p1_nees.sci_mpls(iv), p1_nees.ekf_mpls(iv));
end

fprintf('\n--- Phase 2: v = %d m/s ---\n', v_ref);
fprintf('%-8s | %-8s %-8s %-8s %-8s %-8s %-8s | %-8s %-8s\n', ...
    'T(s)', 'SCI', 'EKF', 'EKFinf', 'SDS', 'CI', 'DR', 'ANEES_S', 'ANEES_E');
fprintf('----------------------------------------------------------------------\n');
for jf = 1:nF
    fprintf('%-8.3f | %-8.3f %-8.3f %-8.3f %-8.3f %-8.3f %-8.3f | %-8.2f %-8.2f\n', ...
        T_round_values(jf), ...
        p2_rmse.sci_mpls(jf), p2_rmse.ekf_mpls(jf), ...
        p2_rmse.ekf_inflate_mpls(jf), p2_rmse.sds_twr_sci(jf), ...
        p2_rmse.ci_mpls(jf), p2_rmse.dr(jf), ...
        p2_nees.sci_mpls(jf), p2_nees.ekf_mpls(jf));
end

%% ========================================================================
%% 18. Clean up temporary data directory
%% ========================================================================
...
%% Plot_E3_FromMat is the single source of truth for the result figures
%% (combined 2x2 RMSE/ANEES panel, coverage, time series, scene overview);
%% it reloads the latest results_E3_case2_*.mat.
%% ========================================================================
Plot_E3_FromMat;
...
%% ========================================================================
function [metrics, dbg, traj] = run_e3_condition(target_v, T_round, ...
    N_sim, scenario_id, node_count, T_total_val, settings, ...
    temp_dir, print_params)

    methods_list = {'sci_mpls','ekf_mpls','ekf_inflate_mpls','sds_twr_sci','ci_mpls','dr'};
    nM_loc = length(methods_list);

    % Flat arrays for parfor compatibility (no dynamic struct fields inside parfor)
    % index: 1=SCI+MPLS, 2=EKF+MPLS, 7=EKF+inflate+MPLS, 4=SDS-TWR+SCI, 5=CI+MPLS, 6=DR
    ...
    mc_rmse_7   = zeros(1, N_sim);  mc_nees_7   = zeros(1, N_sim);  mc_n95_7 = zeros(1, N_sim);
    ...
    % dbg/traj collected via cell arrays (parfor-compatible)
    dbg_cell  = cell(1, N_sim);
    traj_cell = cell(1, N_sim);

    % Print params once outside parfor
    if print_params
        param_tmp = [];
        param_tmp.N = node_count;
        param_tmp.scenario = scenario_id;
        param_tmp.anchors_mobile = true;
    ...
    parfor sim_idx = 1:N_sim
        ...
        param.anchors_mobile = true;   % E3: all nodes mobile
        ...
        % --- MPLS front-end: write directly to unique path (avoids copyfile) ---
        data_file = fullfile(temp_dir, ...
            sprintf('E3_v%d_T%d_run%d.mat', ...
        ...
        % Shared seed: all methods start from identical initial conditions
        base_seed = sim_idx * 1000;

        % --- SCI + MPLS ---
        rng(base_seed);
        [v1, ~, d1] = SCI_Main_AnchorFree_function(data_file, 'MPLS', 'SCI');
        ...
        % --- EKF + MPLS ---
        rng(base_seed);
        [v2, ~, d2] = SCI_Main_AnchorFree_function(data_file, 'MPLS', 'EKF');
        ...
        % --- EKF+inflate + MPLS ---
        rng(base_seed);
        [v7, ~, d7] = SCI_Main_AnchorFree_function(data_file, 'MPLS', 'EKF_inflate');
        mc_rmse_7(sim_idx) = v7;
        mc_nees_7(sim_idx) = d7.mean_nees;
        mc_n95_7(sim_idx)  = d7.nees_within95_ratio;

        % --- SDS-TWR + SCI ---
        rng(base_seed);
        [v4, ~, d4] = SCI_Main_AnchorFree_function(data_file, 'SDS_TWR', 'SCI');
        ...
        % --- CI + MPLS ---
        rng(base_seed);
        [v5, ~, d5] = SCI_Main_AnchorFree_function(data_file, 'MPLS', 'CIEKF');
        mc_rmse_5(sim_idx) = v5;
        mc_nees_5(sim_idx) = d5.mean_nees;
        mc_n95_5(sim_idx)  = d5.nees_within95_ratio;

        % --- Dead Reckoning (prediction only) ---
        rng(base_seed);
        [v6, ~, d6] = SCI_Main_AnchorFree_function(data_file, 'Raw', 'DR');
        mc_rmse_6(sim_idx) = v6;
        mc_nees_6(sim_idx) = d6.mean_nees;
        mc_n95_6(sim_idx)  = d6.nees_within95_ratio;

        % --- Save representative debug/traj data (last MC run) ---
        if sim_idx == N_sim
            dbg_cell{sim_idx} = struct('sci_mpls', d1, 'ekf_mpls', d2, ...
                'ekf_inflate_mpls', d7, 'sds_twr_sci', d4, ...
                'ci_mpls', d5, 'dr', d6);
    ...
    % --- Pack into struct matching caller interface ---
    mc_rmse_all   = {mc_rmse_1, mc_rmse_2, mc_rmse_7, mc_rmse_4, mc_rmse_5, mc_rmse_6};
    mc_nees_all   = {mc_nees_1, mc_nees_2, mc_nees_7, mc_nees_4, mc_nees_5, mc_nees_6};
    mc_n95_all    = {mc_n95_1, mc_n95_2, mc_n95_7, mc_n95_4, mc_n95_5, mc_n95_6};
