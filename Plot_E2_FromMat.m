%% Plot_E2_FromMat.m - E2 plot-only script (loads saved .mat, no simulation)
% Data source: latest results_E2_case2_*.mat (auto-selected; MC=100, Case 2)

clear; close all; clc;

%% 1. Load data
d_ = dir(fullfile('matfile', 'results_E2_case2_*.mat'));
assert(~isempty(d_), 'no results_E2_case2_*.mat in matfile/');
[~, ix_] = max([d_.datenum]);
...
%% --- ANEES: median + IQR. NEES is right-skewed at envelope edges (occasional
%% divergence), so median+IQR is used instead of mean+/-sigma.
%% RMSE: MC mean, no error band (paper convention).
%% Fallback to mean+/-sigma for ANEES if raw per-trial data unavailable. ---
...
%% 3. Plotting configuration (5 methods)
plot_methods = {'sds_twr_sci', 'ekf_mpls', 'ekf_inflate_mpls', 'ci_mpls', 'sci_mpls'};
nM_plot = length(plot_methods);
line_styles  = {'k:^',  'c-d',  'g-.v', 'm-*',  'r-s'};
line_colors  = {'k',    'c',    'g',    'm',    'r'};
line_widths  = [1.0,    1.0,    1.0,    1.0,    2.0];
marker_sizes = [7,      8,      8,      9,      11];
fill_colors  = {'none', 'c',    'g',    'm',    'r'};
legend_names = {'SDS-TWR+SCI', 'EKF+MPLS', 'EKF+Inflate+MPLS', ...
                'CI+MPLS', 'SCI+MPLS (Proposed)'};
leg_order = [5, 4, 3, 2, 1];
...
% Raw .mat retains v=5 column for audit; excluded from plots/tables only.
v_plot_idx = (1 + double(velocities(1) == 5)):nV;   % skip first if it's v=5
velocities_plot = velocities(v_plot_idx);

% Error bands: only for methods with stable std/mean ratio
band_idx = [4, 5];  % ci_mpls, sci_mpls

%% ========================================================================
%% Combined 2x2 paper figure: RMSE (a,b) + ANEES (c,d), one shared legend
%% Single PDF with panel titles; panels (b)/(c)/(d) replace former F2/F3/F4.
%% ========================================================================
ttl_fs = 16; lbl_fs = 13; tr_lbl_fs = 14; ax_fs = 12; leg_fs = 12;
% tr_lbl_fs > lbl_fs: compensates TeX subscript shrink in "T_{round}" so
% the label appears the same size as "Target Velocity (m/s)".
band_alpha = 0.12;
...
% ---- (a) RMSE vs Velocity (log Y; SDS-TWR fits on the log axis, no inset) ----
...
for mi = 1:nM_plot
    ...
    h_plots(mi) = plot(ax_a, velocities_plot, p1_rmse.(mn)(v_plot_idx), line_styles{mi}, ...
        'LineWidth', line_widths(mi), 'MarkerSize', marker_sizes(mi), 'MarkerFaceColor', fill_colors{mi});
end
...
lgd = legend(ax_a, [h_plots(leg_order); h_ideal], [legend_names(leg_order), {'Ideal'}], ...
    'Orientation','horizontal', 'NumColumns',3, 'FontSize',leg_fs, 'FontName','Times New Roman');
lgd.Layout.Tile = 'south';

% Align T_{round} x-label vertically with the velocity x-label on the same row.
drawnow;
...
save_fig(fig, 'E2_RMSE_ANEES');
save_pdf(fig, 'E2_RMSE_ANEES');
close(fig);

% Panels (b)/(c)/(d) subsume former separate F2/F3/F4.
...
title(sprintf('E2: NEES 95%% Coverage vs Velocity (T_{round}=%.3fs)', T_round_ref), ...
    'FontSize', 14, 'FontName', 'Times New Roman');
legend([h_plots(leg_order); h_ideal], [legend_names(leg_order), {'Ideal'}], 'Location', 'best', 'FontSize', 11);
...
ylabel('Fraction within 95% NEES interval', 'FontSize', 12, 'FontName', 'Times New Roman');
title(sprintf('E2: NEES 95%% Coverage vs T_{round} (v=%d m/s)', v_ref), ...
    'FontSize', 14, 'FontName', 'Times New Roman');
legend([h_plots(leg_order); h_ideal], [legend_names(leg_order), {'Ideal'}], 'Location', 'best', 'FontSize', 11);
...
if ~isempty(cross_dbg) && isfield(cross_dbg, 'sci_mpls')
    ...
    subplot(2,1,1);
    hold on;
    h_sds  = plot(t_axis, d_raw.mean_pos_err_time_series,  'k:',   'LineWidth', 1.0);
    ...
    title(sprintf('(a) Error Time Series (v=%d m/s, T_{round}=%.3fs)', ...
        v_ref, T_round_ref), 'FontSize', 13, 'FontName', 'Times New Roman');
    legend([h_sci, h_ci, h_einf, h_ekf, h_sds], ...
        {'SCI+MPLS (Proposed)', 'CI+MPLS', 'EKF+Inflate+MPLS', 'EKF+MPLS', 'SDS-TWR+SCI'}, ...
    ...
    h_sds  = semilogy(t_axis, d_raw.nees_time_series,  'k:',   'LineWidth', 1.0);
    ...
    title(sprintf('(b) ANEES Time Series (v=%d m/s, T_{round}=%.3fs)', ...
        v_ref, T_round_ref), 'FontSize', 13, 'FontName', 'Times New Roman');
    legend([h_fill, h_sci, h_ci, h_einf, h_ekf, h_sds, h_ideal], ...
        {'95% Interval', 'SCI+MPLS', 'CI+MPLS', 'EKF+Inflate+MPLS', ...
         'EKF+MPLS', 'SDS-TWR+SCI', 'Ideal'}, ...
...
%% ========================================================================
%% F8: Scenario Overview (cross-point trajectories)
%% ========================================================================
if ~isempty(cross_traj)
    ...
    if is_case2
        ...
        % Cluster A (nodes 3-5): orbit range x=[155,245], y=[115,195]
        ...
        % Cluster B (nodes 6-8): orbit range x=[255,345], y=[305,395]
    ...
    % --- Communication links (mid-simulation snapshot; 2 nearest in-range neighbors) ---
    ...
    % --- Anchors ---
    Anchors_Pos = cross_traj.Anchors_Pos;
    Anchors_ID  = cross_traj.Anchors_ID;
    if ~isempty(Anchors_Pos)
        plot(Anchors_Pos(1,:), Anchors_Pos(2,:), 'gs', ...
            'MarkerSize', 14, 'MarkerFaceColor', 'g', 'LineWidth', 2, ...
            'HandleVisibility', 'off');
        for ai = 1:size(Anchors_Pos, 2)
            text(Anchors_Pos(1,ai)+10, Anchors_Pos(2,ai)+10, ...
                sprintf('A%d', Anchors_ID(ai)), 'FontSize', 10, ...
                'Color', [0 0.5 0]);
        end
    end

    % --- Agent trajectories with traj_type labels ---
    agent_nodes = setdiff(1:param_plot.N, Anchors_ID);
    colors_agent = lines(length(agent_nodes));
    traj_type_names = {'Stat', 'Circ', 'Ellip', 'Liss', 'Rect', 'Uturn'};
    if isfield(param_plot, 'traj_type')
        agent_traj_types = param_plot.traj_type(agent_nodes);
    else
        agent_traj_types = ones(1, length(agent_nodes));
    end

    for ai = 1:length(agent_nodes)
        ni = agent_nodes(ai);
        tr = squeeze(cross_traj.Xtrue_all(:, ni, :));
        plot(tr(1,:), tr(2,:), '-', 'Color', colors_agent(ai,:), ...
            'LineWidth', 1.0, 'HandleVisibility', 'off');
        plot(tr(1,1), tr(2,1), 'o', 'Color', colors_agent(ai,:), ...
            'MarkerSize', 6, 'MarkerFaceColor', colors_agent(ai,:), ...
            'HandleVisibility', 'off');

        tt = agent_traj_types(ai);
        ...
        text(tr(1,1)+8, tr(2,1)+8, ...
            ...
            'FontSize', 9, 'Color', colors_agent(ai,:));
    end
        ...
        if tt == 4 || tt == 5
            ...
            if norm_d > 0.1
                quiver(x_arr, y_arr, dx*18/norm_d, dy*18/norm_d, 0, ...
                    'Color', colors_agent(ai,:), 'LineWidth', 1.8, ...
    ...
    title(sprintf('E2: Scenario Overview (v=%d m/s)', v_ref), ...
...
function fill_band_lohi(ax, x, y_lo, y_hi, color, alpha_val)
    % Fill asymmetric [lo,hi] band (handles IQR on log axis).
    y_lo = max(y_lo, 1e-10);
    fill_x = [x(:); flipud(x(:))];
...
function q = pctl_(v, p)
    % Linear-interpolation percentile, no toolbox required (p in [0,100]).
    v = sort(v(:));
    n = numel(v);
...
function [med, lo, hi] = robstat_(M)
    % Per-row median (P50), P25, P75 over MC trials; NaN-safe.
